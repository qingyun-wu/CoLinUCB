...
class simulateOnlineData():
	...
	def getThetaAbsDiff(self, user, alg):
		return sum(map(abs, user.theta, alg.getLearntParameters(user.id)))
	def getCoThetaAbsDiff(self, user, alg):
		return sum(map(abs, user.CoTheta, alg.getLearntParameters(user.id)))
	...
	def runAlgorithms(self, algorithms):
		...
		for alg_name, alg in algorithms.items():
			print len(tim_[alg_name]), len(BatchAverageRegret[alg_name])		
			plt.plot(tim_[alg_name], BatchAverageRegret[alg_name], label = alg_name)
			plt.legend()
